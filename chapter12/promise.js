...
정해진 장시간의 기능을 수행하고 나서 정상적으로 기능이 수행되었다면, 성공의 메세지와 함께 처리된 결과값 전달
...
promise의 2가지 공부 포인트
1. state(상태) - process가 무거운 operation을 수행하고 있는 중인지, 기능 수행이 완료되어 성공 or 실패했는지
- state : operation이 수행중일 때 - pending -> 완료된 상태 (fulfilled) or 파일을 찾을 수 없거나 네트워크에 문제가 있는 상태 (rejected)
...
// then() : promise가 정상적으로 수행되면 최종적으로 resolve라는 콜백함수에서 전달된 ellie라는 value가 들어옴
promise
    .then((value) =>  {
        console.log(value);
        /*
