...
let number2 = age;
console.log(`age : ${age}`); // age : 2
console.log(`number2 : ${number2}`); // number2 : 2

number2 = 3;
console.log(`age : ${age}`); // age : 2
console.log(`number2 : ${number2}`); // number2 : 3

// object
// name과 age라는 값들을 하나로 묶어서 객체화, obj는 데이터가 있는 주소를 포함하고 있음
// 😑 const는 값이 한번 할당되면 안 바뀌기 때문에 
// obj 안의 references들을 다른 object로 변경이 안됨
const obj = {
    name : 'minjung',
    age : 27,
};
...
*/
// 그런데, 다음과 같이는 가능함!
// ref라는 공간은 잠겨있지만, object 내부의 ref가 가리키는 것을 이용해 
// ref 안의 값을 변경하는 것은 가능하기 때문
obj.name = 'kitty';
console.log(obj.name); // kitty
// obj라는 값이 가리키는 주소로 찾아감 -> 주소 안에 name이라는 key의 value인 kitty를 출력
